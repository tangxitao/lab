...
del df1["Time"]
df1 = df1.rename(columns={'总使用率': 'cpu1_usage'})
df2=pd.read_csv("CPU使用率3.csv")
del df2["Time"]
df2 = df2.rename(columns={'总使用率': 'cpu3_usage'})
df3 =pd.read_csv("CPU使用率5.csv")
del df3["Time"]
df3 = df3.rename(columns={'总使用率': 'cpu5_usage'})
df=pd.concat([df1,df2,df3],axis=1)

df1 =pd.read_csv("每秒磁盘读写容量1.csv")
del df1["Time"]
df1 = df1.rename(columns={'sda_写入': 'sda_1'})
df2=pd.read_csv("每秒磁盘读写容量3.csv")
del df2["Time"]
df2 = df2.rename(columns={'sda_写入': 'sda_3'})
df3 =pd.read_csv("每秒磁盘读写容量5.csv")
del df3["Time"]
df3 = df3.rename(columns={'sda_写入': 'sda_5'})
